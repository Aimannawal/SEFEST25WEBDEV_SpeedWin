...
<body class="font-poppins bg-gray-100 min-h-screen flex">
    ...
    <main class="flex-1 overflow-y-auto">
        ...
        <div class="container mx-auto px-6 py-8">
            <h1 class="text-3xl font-bold text-gray-800 mb-4">Edit Job Posting</h1>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden p-8">
                ...
                <form action="{{ route('admin.jobs.update', $job->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="title" name="title" class="w-full px-3 py-2 border border-gray-300 rounded-md">
                                <option value="{{ $job->title }}" {{ $job->title }}>{{ $job->title }}</option>
                        ...
                        <div>
                            ...
                            <input type="text" id="company" name="company" value="{{ $job->company }}" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>
                        </div>
                        <div>
                            <label for="location" class="block text-sm font-medium text-gray-700 mb-1">Location</label>
                            <input type="text" id="location" name="location" value="{{ $job->location }}" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input type="text" id="salary_range" name="salary_range" value="{{ $job->salary_range }}" class="w-full px-3 py-2 border border-gray-300 rounded-md" placeholder="e.g. $50,000 - $70,000">
                    </div>
                    
                    <div>
                        <label for="application_deadline" class="block text-sm font-medium text-gray-700 mb-1">Application Deadline</label>
                        <input type="date" id="application_deadline" name="application_deadline" value="{{ $job->application_deadline }}" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>
                    </div>
                    
                    <div>
                        <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Job Description</label>
                        <textarea id="description" name="description" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>{{ $job->description }}</textarea>
                    </div>
                    
                    <div>
                        <label for="responsibilities" class="block text-sm font-medium text-gray-700 mb-1">Responsibilities</label>
                        <textarea id="responsibilities" name="responsibilities" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>{{ $job->responsibilities }}</textarea>
                    </div>
                    
                    <div>
                        <label for="qualifications" class="block text-sm font-medium text-gray-700 mb-1">Qualifications</label>
                        <textarea id="qualifications" name="qualifications" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>{{ $job->qualifications }}</textarea>
                    </div>
